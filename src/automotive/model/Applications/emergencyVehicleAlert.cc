...
namespace ns3
{
  ...
  emergencyVehicleAlert::GetTypeId(void)
  {
    static TypeId tid =
        TypeId("ns3::emergencyVehicleAlert")
            ...
            .AddAttribute(
                "HarmMetric",
                "Canonical harm metric minimized by the optimizer AND reported by the "
                "HarmLogger / figures (Fix 1 — they must agree). 'deltaV' (default) is the "
                "paper formula (3) m_other/(m_self+m_other)*|v_rel| [kg-norm m/s]; "
                "'kinetic_energy' is reduced-mass ½·mu·v_rel² [J].",
                StringValue("deltaV"),
                MakeStringAccessor(&emergencyVehicleAlert::m_harm_metric_name),
                MakeStringChecker())
            .AddAttribute(
                "EthicalWeightLead",
                "Fix 5: priority weight W applied to the harm the middle vehicle would suffer "
                "from the leader collision (H_{2,1}) in the optimizer's H = Σ Wᵢ Hᵢ. "
                "1.0 reproduces the unweighted sum.",
                DoubleValue(1.0),
                MakeDoubleAccessor(&emergencyVehicleAlert::m_w_lead),
                MakeDoubleChecker<double>(0.0))
            .AddAttribute(
                "EthicalWeightFollow",
                "Fix 5: priority weight W applied to the harm the follower would suffer from "
                "the rear collision (H_{3,2}) in the optimizer's H = Σ Wᵢ Hᵢ. "
                "1.0 reproduces the unweighted sum.",
                DoubleValue(1.0),
                MakeDoubleAccessor(&emergencyVehicleAlert::m_w_follow),
                MakeDoubleChecker<double>(0.0))
            .AddAttribute(
                "Restitution",
                "Fix 14: coefficient of restitution e ∈ [0,1] for collisions. 0 = perfectly "
                "inelastic (default, conservative). Drives post-collision velocities "
                "(momentum + restitution) and the chain's SECOND collision included in "
                "H_total.",
                DoubleValue(0.0),
                MakeDoubleAccessor(&emergencyVehicleAlert::m_restitution),
                MakeDoubleChecker<double>(0.0, 1.0))
            .AddAttribute(
                "LinkPacketErrorRate",
                "Fix 14: assumed packet-error rate p ∈ [0,1] used by SigmaMode='linkquality' "
                "to stretch the decision budget σ as the channel degrades (Ethical-V2X §V). "
                "Ignored for the other σ modes.",
                DoubleValue(0.1),
                MakeDoubleAccessor(&emergencyVehicleAlert::m_link_per),
                MakeDoubleChecker<double>(0.0, 1.0))
            .AddAttribute(
                "LinkSigmaGain",
                "Fix 14: gain multiplying the link-quality σ stretch. σ = σ_computed * "
                "(1 + LinkSigmaGain * p/(1-p)) when SigmaMode='linkquality'.",
                DoubleValue(1.0),
                MakeDoubleAccessor(&emergencyVehicleAlert::m_link_sigma_gain),
                MakeDoubleChecker<double>(0.0))
            .AddAttribute(
                "DENMMrcCombining",
                "Fix 14 (scoped, knob only): intent flag for MRC/diversity-combining of DENM "
                "copies. Not yet modeled — see code TODO; exposed so campaigns can pin it.",
                BooleanValue(false),
                MakeBooleanAccessor(&emergencyVehicleAlert::m_denm_mrc_combining),
                MakeBooleanChecker())
            .AddAttribute(
                "HeterogeneousTraffic",
                "Fix 14 (scoped, knob only): intent flag for cross / oncoming-traffic role "
                "logic. Not yet modeled — see code TODO; exposed so scenarios can pin it.",
                BooleanValue(false),
                MakeBooleanAccessor(&emergencyVehicleAlert::m_heterogeneous_traffic),
                MakeBooleanChecker());
    return tid;
  }
  ...
  emergencyVehicleAlert::emergencyVehicleAlert()
  {
    ...
    m_fixed_sigma = 0.5;
    m_harm_metric_name = "deltaV";
    m_harm_metric = HarmMetric::kDeltaV;
    m_w_lead = 1.0;
    m_w_follow = 1.0;
    m_restitution = 0.0;
    m_link_per = 0.1;
    m_link_sigma_gain = 1.0;
    m_denm_mrc_combining = false;
    m_heterogeneous_traffic = false;
  ...
  emergencyVehicleAlert::StartApplication(void)
  {
    ...
    /* Fix 1: resolve the canonical harm metric once. */
    m_harm_metric = ParseHarmMetric(m_harm_metric_name);

    /* Fix 5: source this vehicle's mass from its SUMO vType instead of the
       single VehicleMass attribute, so mass heterogeneity is exercised. Falls
       back to the VehicleMass attribute if TraCI reports nothing usable. */
    m_vehicle_mass = GetVehicleMass(m_id);
                                            ...
                                            SignalInfo sigInfo)
  {
    ...
    // Fix 1: score the per-message harm with the SAME canonical metric the
    // optimizer minimizes and the HarmLogger reports (default deltaV).
    double harm = appUtil_harm(m_harm_metric, m_vehicle_mass, receiver_speed,
                               m_vehicle_mass, sender_speed);
  ...
  // Fix 10: number of DENMs the reliability burst emits == the requested copy
  // count (>=1). The DEN repetition mechanism fires the first copy at the
  // trigger and one more every spacing within the burst window.
  uint32_t
  emergencyVehicleAlert::DenmBurstCopies(uint32_t copies)
  {
    return (copies < 1) ? 1u : copies;
  }

  // Fix 10: burst end time [ms] after the trigger. duration is set just past
  // the last copy (+spacing/2 margin) and must stay below the 500 ms UpdateDenm
  // cadence so the service repetition and the periodic update never overlap.
  double
  emergencyVehicleAlert::DenmBurstWindowMs(uint32_t copies, double spacingMs)
  {
    if (copies <= 1)
      return 0.0;
    return (copies - 1) * spacingMs + spacingMs / 2.0;
  }
  ...
  emergencyVehicleAlert::TriggerDenm(long causeCode, long subCauseCode)
  {
    ...
    if (m_denm_copies > 1)
    {
      ...
      long duration = (long)DenmBurstWindowMs(m_denm_copies, m_denm_copy_spacing_ms);
    ...
    if ((m_ethical_braking_enabled || m_cooperative_detection_enabled) && m_include_ethical_alacarte)
    {
  ...
  // Fix 5: per-vehicle mass from the SUMO vType. TraCI in this tree has no
  // getMass() shortcut, so we read the generic "mass" parameter (SUMO exposes
  // the vType mass under this key). Returns m_vehicle_mass (the VehicleMass
  // attribute default) when the parameter is absent / unparsable, so equal-mass
  // scenarios reproduce the prior constant-mass behavior.
  double
  emergencyVehicleAlert::GetVehicleMass(const std::string &vehicleId)
  {
    try
    {
      std::string raw = m_client->TraCIAPI::vehicle.getParameter(vehicleId, "mass");
      if (!raw.empty())
      {
    ...
    {
      // Fall through to the attribute default.
    }
    return m_vehicle_mass;
  }

  // Fix 1/14.1: solve the piecewise braking kinematics for the (follower,
  // ahead) pair and return the closing relative speed at the collision instant
  // (0 if they never collide). Separated from the harm scoring so the optimizer
  // and every logger can score the SAME canonical metric on this v_rel.
  double
  emergencyVehicleAlert::CalculateCollisionVrel(double v_follower, double a_follower,
                                                double v_ahead, double a_ahead, double gap)
  {
    ...
    {
      double gap_at_phase2_start = gap + v_ahead * t_stop_ahead - 0.5 * a_ahead * t_stop_ahead * t_stop_ahead - (v_follower * t_stop_ahead - 0.5 * a_follower * t_stop_ahead * t_stop_ahead);
    ...
    return (v_rel > 0.0) ? v_rel : 0.0;
  }

  // Fix 1: harm the follower suffers colliding with the vehicle ahead, scored
  // with the SELECTED canonical metric (m_harm_metric) on the collision-instant
  // v_rel — so the optimizer minimizes exactly what the HarmLogger / figures
  // report. Default metric is deltaV (paper formula 3).
  double
  ...
  {
    double v_rel = CalculateCollisionVrel(v_follower, a_follower, v_ahead, a_ahead, gap);
    if (v_rel <= 0.0)
      return 0.0;
    return appUtil_harmFromVrel(m_harm_metric, m_follower, m_ahead, v_rel);
  }
  ...
  emergencyVehicleAlert::CalculateOptimalDeceleration(double v1, double a1, double m1,
                                                      ...
                                                      double sigma)
  {
    // Fix 7: model the σ-wait. During σ the middle vehicle (V2) coasts at v2
    // while the leader (V1) brakes at a1; V3 has not yet reacted, so it also
    // coasts (conservative). Advance speeds/gaps by σ, then evaluate harm with
    // braking starting from the advanced state. σ=0 reproduces the prior
    // "brake now" objective.
    if (sigma < 0.0)
      sigma = 0.0;
    double v1_s = v1, v2_s = v2, v3_s = v3;
    double gap12_s = gap12, gap23_s = gap23;
    if (sigma > 0.0)
    {
      double t1 = (a1 > 0.0) ? std::min(sigma, v1 / a1) : sigma;
      double d1 = v1 * t1 - 0.5 * a1 * t1 * t1; // leader brakes during σ
      double d2 = v2 * sigma;                   // V2 coasts during σ
      double d3 = v3 * sigma;                   // V3 coasts during σ
      v1_s = std::max(0.0, v1 - a1 * t1);
      gap12_s = std::max(0.0, gap12 + d1 - d2); // gap V2→V1
      gap23_s = std::max(0.0, gap23 + d2 - d3); // gap V3→V2
    }

    double best_a2 = a2_max;
    double min_harm = std::numeric_limits<double>::max();

    // Fix 7: sweep a2 from 0 (coasting allowed) to a2_max in 0.1 m/s^2 steps.
    for (double a2 = 0.0; a2 <= a2_max + 0.05; a2 += 0.1)
    {
      double clamped_a2 = std::min(a2, a2_max);

      // H_{2,1}: V2 follows V1. v_rel at impact drives both the harm and the
      // post-impact velocity used for the chain's second collision.
      double vrel12 = CalculateCollisionVrel(v2_s, clamped_a2, v1_s, a1, gap12_s);
      double h12 = appUtil_harmFromVrel(m_harm_metric, m2, m1, vrel12);

      // H_{3,2}: V3 follows V2. Fix 14.2 — if V2 already hit V1, V2's post-
      // impact velocity (1D momentum + restitution e) becomes the speed V3
      // sees, so the follow-on collision in the chain is captured in H_total.
      double v2_for_follow = v2_s;
      if (vrel12 > 0.0)
      {
        // V2 (mass m2, speed v2_s) hits V1 (mass m1, speed v1_s = v2_s - vrel12
        // at impact direction). Post-impact V2 speed from conservation of
        // momentum with coefficient of restitution m_restitution.
        double v1_impact = v2_s - vrel12;
        double total = m1 + m2;
        if (total > 0.0)
          v2_for_follow =
              (m2 * v2_s + m1 * v1_impact - m_restitution * m1 * vrel12) / total;
        v2_for_follow = std::max(0.0, v2_for_follow);
      }
      double vrel23 = CalculateCollisionVrel(v3_s, a3, v2_for_follow, clamped_a2, gap23_s);
      double h23 = appUtil_harmFromVrel(m_harm_metric, m3, m2, vrel23);

      // Fix 5: weighted total harm H = Σ Wᵢ Hᵢ.
      double h_total = m_w_lead * h12 + m_w_follow * h23;
  ...
  emergencyVehicleAlert::HandleCooperativeDenm(denData &denm, unsigned long senderStationId)
  {
    ...
    try
    {
      ...
      double leader_mass = m_vehicle_mass; // Fix 5: per-vehicle, from SUMO vType
      ...
      if (has_leader)
      {
        try
        {
          ...
          leader_mass = GetVehicleMass(leader.first);
      ...
      double follower_mass = m_vehicle_mass; // Fix 5: refined from SUMO vType below
      ...
      // Fix 5: per-vehicle follower mass from its SUMO vType (id == "veh"+id).
      if (has_follower)
      {
        try
        {
          follower_mass = GetVehicleMass("veh" + std::to_string(follower_station_id));
        }
        catch (...)
        {
        }
      ...
      if (m_coopBraking.active && !m_coopBraking.decisionMade && sender_is_behind)
      {
        ...
        // Rear DENM arrived within σ, so the decision is "made now": no
        // residual σ-wait to advance state over (sigma=0).
        double optimal_a2 = CalculateOptimalDeceleration(
            v1, a1, m1_mass, my_speed, m_vehicle_mass, my_max_decel,
            v3, a3, m3_mass, gap_to_leader, gap_to_follower, 0.0);
      ...
      if (originator_is_leader && has_follower && !m_coopBraking.active)
      {
        ...
        else if (m_sigma_mode == "linkquality")
        {
          // Fix 14.3: a worse channel (higher packet-error rate p) buys a
          // longer decision budget, since fresh DENM/CAM updates arrive less
          // reliably. σ = σ_computed * (1 + gain * p/(1-p)), saturating as p→1.
          double p = std::min(0.999, std::max(0.0, m_link_per));
          double stretch = 1.0 + m_link_sigma_gain * (p / (1.0 - p));
          sigma = std::max(0.0, sigma_computed * stretch);
        }
        else
          sigma = sigma_computed;
        m_coopBraking.sigma = sigma;

        // Fix 6/7: closed-form suboptimal deceleration used as the EXPLICIT
        // fallback when the σ-timeout has no follower state (see
        // CooperativeDecisionTimeout). It guarantees H_{2,1}=0 while accounting
        // for the gap that V2 closes coasting during σ: the distance available
        // to brake in is (gap + leader_stop_dist - v2*σ).
        double leader_stop_dist = leader_speed * leader_speed / (2.0 * a_ahead + 1e-9);
        double brake_room = gap_to_leader + leader_stop_dist - my_speed * sigma;
        double subopt_a2 = (brake_room > 0.0)
                               ? my_speed * my_speed / (2.0 * brake_room)
                               : my_max_decel;
      ...
      if (!originator_is_leader && !m_coopBraking.active)
      {
        ...
        double h_ahead = has_leader ? CalculateHarm(m_vehicle_mass, my_speed, chain_decel,
  ...
  emergencyVehicleAlert::CooperativeDecisionTimeout()
  {
    ...
    double leader_max_decel = m_coopBraking.leaderAheadDecel > 0
                                  ? m_coopBraking.leaderAheadDecel
                                  : 7.5;
    ...
    bool have_follower_state = false;
    if (m_coopBraking.followerStationId != 0)
    {
      try
      {
        follower_mass =
            GetVehicleMass("veh" + std::to_string(m_coopBraking.followerStationId));
      }
      catch (...)
      {
      }
    ...
    // Fix 6: Paper Alg. 2 line 12 fallback. With no follower state there is no
    // H_{3,2} term to trade off, so the joint optimizer would just collapse to
    // "minimize H_{2,1}" — exactly the closed-form suboptimalDecel computed
    // when we entered the wait (which already reserves the σ-coast distance).
    // Use it explicitly here instead of recomputing; otherwise run the σ=0
    // joint optimization on the fresh post-σ state.
    double optimal_a2;
    if (!have_follower_state)
    {
      optimal_a2 = m_coopBraking.suboptimalDecel;
    }
    else
    {
      // σ has already physically elapsed by the time this timeout fires, and the
      // leader gap/speed are re-queried fresh from TraCI, so the optimizer sees
      // the post-σ state directly: pass sigma=0 (do not double-advance).
      optimal_a2 = CalculateOptimalDeceleration(
          ...
          gap_to_leader, gap_to_follower, 0.0);
    }
    ...
    NS_LOG_INFO("[" << Simulator::Now().GetSeconds() << "] Vehicle " << m_id
                    << " COOPERATIVE: sigma timeout, "
                    << (have_follower_state ? "joint optimization on last-CAM data"
                                            : "closed-form suboptimal fallback")
                    << " a2=" << optimal_a2
                    << " H12=" << h12 << " H23=" << h23
                    << " Htotal=" << h12 + h23);
